...
namespace katachi.Controllers
{
    ...
    {
        [HttpGet]
        public IActionResult GetMuscleAnalysis(string month)
        {
            ...
            using (var cmd = conn.CreateCommand())
            {
                cmd.CommandText = @"
                    ...
                    WHERE ts.user_id = @uid AND ti.is_deleted = 0
            ...
            using (var cmd = conn.CreateCommand())
            {
                cmd.CommandText = "SELECT group_key, monthly_target FROM user_goals WHERE user_id = @uid AND month = @mon";
                cmd.Parameters.AddWithValue("@uid", CurrentUserId);
